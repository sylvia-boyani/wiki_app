class WikiPostsController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @wiki_post.update(wiki_post_params)
        format.html { redirect_to @wiki_post, notice: "Wiki post was successfully updated.", status: :see_other }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to wiki_posts_path, notice: "Wiki post was successfully destroyed.", status: :see_other }
  ...
  private
    ...
    def set_wiki_post
      @wiki_post = WikiPost.find(params.expect(:id))
    end
